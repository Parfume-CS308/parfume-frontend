...
const OrdersPage: React.FC = () => {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const withinDateRange = !startDate || !endDate || isWithinInterval(orderDate, { start: startDate, end: endDate })
    return (filterStatus === 'all' || order.status === filterStatus) && withinDateRange
  })
    ...
    return filteredOrders.reduce((total, order) => {
      return (
        ...
        order.items.reduce((orderTotal, item) => {
          const profit = (item.discountedPrice - item.basePrice) * item.quantity // Calculate profit for each item
  ...
  const renderProfitChart = () => {
    ...
    return (
      <div>
        <h2 className='text-2xl font-bold text-gray-800'>Total Profit by Day</h2>
        <div className='p-16'>
          <Bar
            data={chartData}
            options={{
              scales: {
                y: {
                  beginAtZero: true,
                  min: Math.min(...Object.values(profitByDay)) < 0 ? Math.min(...Object.values(profitByDay)) - 10 : 0,
                  title: {
                    display: true,
                    text: 'Profit ($)'
                  }
        ...
        <div className='mb-8'>
          <h2 className='text-2xl font-bold text-gray-800 text-center'>Total Profit: ${totalProfit.toFixed(2)}</h2>
        </div>
      </div>
    )
  }
  ...
  const renderRevenueChart = () => {
    ...
    return (
      <div>
        <h2 className='text-2xl font-bold text-gray-800'>Total Revenue by Day</h2>
        <div className='p-16'>
          <Bar
            data={chartData}
            options={{
              scales: {
                y: {
                  beginAtZero: true,
                  min: Math.min(...Object.values(revenueByDay)) < 0 ? Math.min(...Object.values(revenueByDay)) - 10 : 0,
                  title: {
                    display: true,
                    text: 'Revenue ($)'
                  }
        ...
        <div className='mb-8'>
          <h2 className='text-2xl font-bold text-gray-800 text-center'>Total Revenue: ${totalRevenue.toFixed(2)}</h2>
        </div>
      </div>
    )
  }
        ...
        {filteredOrders.map(order => (
          <div key={order.orderId} className='bg-white shadow-lg rounded-lg overflow-hidden'>
            ...
            <div className='p-6 bg-gray-50 flex justify-between items-center'>
              ...
              {order.status === OrderStatus.PROCESSING && user?.role === 'product-manager' && (
                <button
      ...
      {selectedOrder && (
        <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='bg-white p-6 rounded-lg shadow-lg'>
            <button onClick={handleCloseInvoice} className='absolute top-2 right-2 text-gray-500'>
              &times;
            </button>
