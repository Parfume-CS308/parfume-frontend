...
import useWishlist from '@/hooks/contexts/useWishlist.tsx'
...
const PerfumeDetailPage: React.FC = () => {
  ...
  const [selectedVariant, setSelectedVariant] = React.useState<PerfumeVariant | null>(null)
  ...
  const setMinimumVariantWithStock = (variants: PerfumeVariant[], id: string) => {
    const variantsWithStock = variants.filter(variant => variant.stock > 0).sort((a, b) => a.volume - b.volume)

    const basketVariant = variants.find(variant => getBasketProduct(id, variant.volume))
    setSelectedVariant(basketVariant || (variantsWithStock.length > 0 ? variantsWithStock[0] : variants[0]))
  }
  // #endregion
      ...
      <>
        <div className='flex items-center gap-2'>
          {perfume?.variants.map(variant => (
            <span
              className={cn('p-4 border-primary border-2 cursor-pointer text-primary', {
                'bg-primary text-white': selectedVariant?.volume === variant.volume
              })}
  ...
  const renderStock = () => {
    return (
      <div className='flex items-center gap-4'>
        <div className='text-gray-600 font-semibold'>Stock: {selectedVariant?.stock}</div>
      </div>
    )
  }
    ...
    return (
      <div>
        <div className='mt-6 space-y-4'>
          ...
          <div className='flex items-center gap-4'>
            <div className='text-2xl font-bold'>${leftPrice}</div>
            {hasDiscount && <div className='text-sm text-gray-500 line-through'>${selectedVariant?.price}</div>}
          </div>
      ...
      <div className='flex justify-between'>
        <div className='flex items-center bg-white p-2 rounded-lg border-2'>
          ...
          <div className='px-8 py-2 border-r flex items-center'>
            {getBasketProduct(id ?? '', selectedVariant?.volume ?? 0)?.quantity || 0}
          </div>
          <Button
            className='px-4 py-2 rounded-r-md'
            onClick={e => {
              e.stopPropagation()
              const basketItem = getBasketProduct(id ?? '', selectedVariant?.volume ?? 0)
              if (basketItem && basketItem.quantity < (selectedVariant?.stock ?? 0)) {
                const item = {
                  perfumeId: id ?? '',
                  perfumeName: perfume?.name ?? '',
                  brand: perfume?.brand ?? '',
  ...
  const renderBasket = () => {
    const basketItem = getBasketProduct(perfume?.id ?? '', selectedVariant?.volume ?? 0)
